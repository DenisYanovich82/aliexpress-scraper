...
def search(keywords="", search_url="", max_product_nb=100, proxy_limit=-1, client=None):
    """Function to get the list of products from amazon"""
    if not client:
        client = AliexpressClient(proxy_limit=proxy_limit)
    product_dict_list = client._get_products(
        keywords=keywords,
        search_url=search_url,
        max_product_nb=max_product_nb)
    products = Products(product_dict_list)
    products.html_pages = client.html_pages
    products.last_html_page = client.html_pages[-1]
